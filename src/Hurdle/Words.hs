...
-- |  The file "assets/allwowed-guesses" contains a broader, less sanitised selection of five-letter words which may be used as guesses.
...
wordFrequencies :: Map.Map String Double
wordFrequencies = Map.fromList $ map (\x->(map toUpper $ head x, read $ head $ tail x)) $ map words $ lines $(embedStringFile "assets/word-relative-frequency")